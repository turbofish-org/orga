...
use merk::{restore::Restorer, rocksdb, BatchEntry, Merk, Op};
...
impl crate::store::Iter for MerkStore {
    type Iter<'a> = Iter<'a>;

    fn iter_from(&self, start: &[u8]) -> Self::Iter<'_> {
        let mut iter = self.merk.as_ref().unwrap().raw_iter();
        iter.seek(start);
        Iter { iter }
    }
}
...
impl ABCIStore for MerkStore {
    ...
    fn query(&self, key: &[u8]) -> Result<Vec<u8>> {
        ...
        let data = self.merk.as_ref().unwrap().prove(val)?;
        hash.extend(data);
        Ok(hash)
    }
    ...
    fn apply_snapshot_chunk(
        ...
        if let None = self.restorer {
            let expected_hash: [u8; 20] = target_snapshot
                .hash
                .clone()
                .try_into()
