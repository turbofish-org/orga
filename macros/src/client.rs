...
fn create_field_adapters(item: &DeriveInput) -> (TokenStream2, Vec<(&Field, ItemStruct)>) {
    ...
    let adapters: Vec<_> = fields
        .iter()
        .enumerate()
        .map(|(i, f)| create_field_adapter(f, i))
        .collect();
...
fn create_field_adapter(field: &Field, i: usize) -> (TokenStream2, ItemStruct) {
    ...
    let struct_def = quote! {
        #[derive(Clone)]
        pub struct #struct_name<#parent_ty: Clone> {
            parent: #parent_ty,
        }
    };
    ...
    let output = quote! {
        #struct_def
        impl#adapter_generics #struct_name<#parent_ty> {
            pub fn new(parent: #parent_ty) -> Self {
                Self {
                    parent,
                }
